package hu.ponte.hr;

import hu.ponte.hr.services.SignService;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.mock.web.MockMultipartFile;
import org.springframework.test.context.junit4.SpringRunner;

import java.io.InputStream;
import java.util.LinkedHashMap;
import java.util.Map;

import static org.assertj.core.api.Assertions.assertThat;

/**
 * @author zoltan
 */
@RunWith(SpringRunner.class)
@SpringBootTest()
public class SignTest {

    @Autowired
    private SignService signService;

    private static final String CAT_FILE = "cat.jpg";
    private static final String ENHANCED_BUZZ_FILE = "enhanced-buzz.jpg";
    private static final String RND_FILE = "rnd.jpg";

    private final Map<String, String> files = new LinkedHashMap<>() {
        {
            put(CAT_FILE, "");
            put(ENHANCED_BUZZ_FILE, "");
            put(RND_FILE, "");
        }
    };

    @Test
    public void testSignFile() {
        assertThat(files.get(CAT_FILE)).isEqualTo(signFile(CAT_FILE));
        assertThat(files.get(ENHANCED_BUZZ_FILE)).isEqualTo(signFile(ENHANCED_BUZZ_FILE));
        assertThat(files.get(RND_FILE)).isEqualTo(signFile(RND_FILE));
    }

    private String signFile(String fileName) {
        String fileExtension = fileName.split("\\.")[1];

        try (InputStream resourceAsStream = getClass().getClassLoader().getResourceAsStream("images/" + fileName)) {
            byte[] bytes = resourceAsStream.readAllBytes();
            MockMultipartFile mockMultipartFile = new MockMultipartFile(fileName, fileName, fileExtension, bytes);
            return signService.sign(mockMultipartFile);
        } catch (Exception e) {
            throw new RuntimeException();
        }
    }

}
